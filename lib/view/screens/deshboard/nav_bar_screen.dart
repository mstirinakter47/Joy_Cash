...
class _NavBarScreenState extends State<NavBarScreen> {
  ...
  void initState() {
    super.initState();
    Get.find<XenuController>().selectHomePage(isUpdate: false);
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
          ...
          child: GetBuilder<XenuController>(builder: (menuController) {
            return Scaffold(
      ...
      child: Column(
        ...
        children: [
          SizedBox(height: Dimensions.NAVBAR_ICON_SIZE, width: Dimensions.NAVBAR_ICON_SIZE,
            child: Image.asset(icon, fit: BoxFit.contain, color: Get.find<XenuController>().currentTab == selectIndex
                  ? Theme.of(context).textTheme.titleLarge.color : ColorResources.nevDefaultColor,
            ),
          ),
          const SizedBox(height: 6.0),
          Text(name, style: TextStyle(color: Get.find<XenuController>().currentTab == selectIndex
